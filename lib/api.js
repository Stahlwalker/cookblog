...
export async function getTotalPostsNumber() {
  const entries = await fetchGraphQL(
    `query {
      postCollection(order: date_DESC, limit: 0) {
        items {
          ${POST_GRAPHQL_FIELDS}
        }
      }
...
export async function getPaginatedPostSummaries(page) {
  ...
  const skip =
    skipMultiplier > 0 ? 2 * skipMultiplier : 0;

    const entries = await fetchGraphQL(  `query {
    postCollection(order: date_DESC, skip: ${skip}, limit: 20) {
      items {
        ${POST_GRAPHQL_FIELDS}
      }
    }
  ...
  const paginatedPostSummaries = entries.data.postCollection
    ...
    : { total: 0, items: [] };
    console.log("apijsfile",paginatedPostSummaries.items);
  return extractPostEntries(paginatedPostSummaries.items)
}
